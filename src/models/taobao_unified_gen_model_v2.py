...
class TaobaoAuGRGenModelV2(PreTrainedModel):
    ...
    def __init__(self, config: TaobaoAuGRGenConfig):
        ...
        if self.sequence_feature_names:
            ...
            # `user_seq_encoder.feature_embeddings` aliases the top-level
            # embedding table; mark alias keys for HF safe serialization.
    ...
    def _forward_hstu(self, user_tokens: torch.Tensor, item_tokens: torch.Tensor):
    ...
    def forward(self, *args, **kwargs):
        ...
        h_user, h_item = self._forward_hstu(user_tokens, item_tokens)

        moe_load_balance_loss = torch.tensor(0.0, device=h_item.device)
        if self.use_post_hstu_moe:
            h_item, moe_load_balance_loss = self.target_moe(h_item)

        if self.ctr_shallow_shortcut:
            h_shallow = self._shallow_encode(user_tokens)
            h_user = h_shallow + h_user
            ctr_input = torch.cat([h_item, h_user], dim=-1)
        else:
            ctr_input = torch.cat([h_item, h_user], dim=-1)
